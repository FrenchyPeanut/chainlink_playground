...
contract ATestnetConsumer is ChainlinkClient, Ownable {
  ...
  uint256 public currentPriceJPY;
  ...
  event RequestEthereumPriceFulfilledJPY(
    bytes32 indexed requestId,
    uint256 indexed price
  );
  ...
  function requestEthereumPriceJPY(address _oracle, string _jobId)
    public
    onlyOwner
  {
    Chainlink.Request memory req = buildChainlinkRequest(stringToBytes32(_jobId), this, this.fulfillEthereumPriceJPY.selector);
    req.add("get", "https://min-api.cryptocompare.com/data/price?fsym=ETH&tsyms=JPY");
    req.add("path", "JPY");
    req.addInt("times", 100);
    sendChainlinkRequestTo(_oracle, req, ORACLE_PAYMENT);
  }
  ...
  function fulfillEthereumPriceJPY(bytes32 _requestId, uint256 _price)
    public
    recordChainlinkFulfillment(_requestId)
  {
    emit RequestEthereumPriceFulfilledJPY(_requestId, _price);
    currentPriceJPY = _price;
  }
